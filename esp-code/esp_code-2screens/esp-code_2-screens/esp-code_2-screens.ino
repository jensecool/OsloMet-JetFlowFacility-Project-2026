...
void kickstart_fan();
void set_fan_speed();
void configure_fan_lcd(int channel);
void configure_pressure_lcd(int channel);
...
// === Configure Fan LCD (channel 0) ===
void configure_fan_lcd(int channel) {
    tca_select(channel);
    delay(10);
    lcd.begin(16, 2);
    lcd.setRGB(255, 255, 0);  // Yellow backlight (fan LCD)
    lcd.setCursor(0, 0);
    lcd.print("Fan ready");
}

// === Configure Pressure LCD (channel 1) ===
void configure_pressure_lcd(int channel) {
    tca_select(channel);
    delay(10);
    lcd.begin(16, 2);
    lcd.setRGB(0, 255, 0);  // Green backlight (pressure LCD)
    lcd.setCursor(0, 0);
...
void control_fan() {
    ...
    char l0[17];
    snprintf(l0, sizeof(l0), "Fan speed: %d%%", fan_speed_percentage);

    tca_select(fan_lcd_channel);
    delay(10);
    lcd.setCursor(0, 0);
    lcd.print("                ");
    lcd.setCursor(0, 0);
...
// === Pressure Control (LCD on channel 1) ===
void control_pressure() {
    float pa = 0.0f;
...
void control_panel_controller(void *pv_parameters) {
    configure_fan_lcd(fan_lcd_channel);
    configure_pressure_lcd(pressure_lcd_channel);
    ...
    while (1) {
        ...
        // If power JUST came back on, re-init both LCDs
        if (!last_power_state) {
            configure_fan_lcd(fan_lcd_channel);
            configure_pressure_lcd(pressure_lcd_channel);
